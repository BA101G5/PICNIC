...
public class PicnicJNDIDAO implements PicnicDAO_interface {
	...
	private static final String K_DELETE_STMT = "delete from PICNIC where PICNIC = ?";
	...
	private static final String K_GET_ALL_STMT_MEM = "SELECT PICNIC.PICNIC_NO,picm.MEM_NO "
			+ "FROM PICNIC left outer JOIN (select PICNIC_NO,MEM_NO from PICMEM where MEM_NO = ?) picm ON PICNIC.PICNIC_NO = picm.PICNIC_NO "
			+ " where PICNIC.ORD_STA != 'U' "
			+ " order by PICNIC.PICNIC_NO ";
	...
	public List<String> k_getAllandMEM(String MEM_NO) {

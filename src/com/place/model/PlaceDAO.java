...
public class PlaceDAO implements PlaceDAO_interface {
	...
	//------------------------------------------------------------------------------------
	//----------------------------------------------------------------------------------------
	private static final String K_INSERT_STMT = "insert into PLACE (P_NO,MF_NO,MEM_NO,P_NAME,P_UNTIL,P_PLACE,P_POP,PIMG,P_INFO,P_STA,P_PRICE,PICNIC_NO,P_LAT,P_LON)values('P'||LPAD(P_NO_SQ.NEXTVAL,9,0),?,?,?,?,?,?,?,?,?,?,?,?,?)";
	private static final String K_GET_ALL_STMT = "select * from PLACE ORDER BY P_NO";
	private static final String K_GET_ONE_STMT = "select MF_NO,MEM_NO,P_NAME,P_UNTIL,P_PLACE,P_POP,PIMG,P_INFO,P_STA,P_PRICE,PICNIC_NO,P_LAT,P_LON from PLACE WHERE P_NO= ?";
	private static final String K_DELETE_STMT = "delete from PLACE where P_NO = ?";
	private static final String K_UPDATE_STMT = "update PLACE set MF_NO=?,MEM_NO=?,P_NAME=?,P_UNTIL=?,P_PLACE=?,P_POP=?,PIMG=?,P_INFO=?,P_STA=?,P_PRICE=?,PICNIC_NO = ?,P_LAT = ?,P_LON = ? where P_NO=?";
	private static final String K_INSERT_FROM_CUST_STMT = "insert into PLACE(P_NO,MEM_NO,P_PLACE,P_STA,PICNIC_NO,P_PRICE,P_LAT,P_LON)values('P'||LPAD(P_NO_SQ.NEXTVAL,9,0),?,?,'I',?,'0',?,?)";
	...
	public void k_insertplace(PlaceVO placeVO) {
		...
		try {
			con=ds.getConnection();
			pstmt = con.prepareStatement(K_INSERT_FROM_CUST_STMT);
			pstmt.setString(1, placeVO.getMem_no());
			pstmt.setString(2, placeVO.getP_place());
			pstmt.setString(3, placeVO.getPicnic_no());
			pstmt.setDouble(4,placeVO.getP_lat());
